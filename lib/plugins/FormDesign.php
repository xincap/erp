...
class FormDesign {
    ...
    public function parse_form($template, $fields = 0) {
                ...
                foreach ($parse_attr[1] as $k => $attr) {
                    ...
                    if ($attr) {
                        ...
                        if ($tag == 'select' && $attr == 'value') {
                            if(!isset($attr_arr_all[$attr])){
                                $attr_arr_all[$attr]    = null;
                            }
         ...
         * start
         */
        $comment  = array_combine(array_column($template_data, 'name'), array_column($template_data, 'title'));
        $data   = [];
        ...
        $parse_form = array(
            ...
            'data' => $template_data,
            'add_fields' => ['fields'=>$add_fields,'comment'=>$data],
        );
    ...
    public function tname($tname) {
        ...
        return config('database.connections.mysql.prefix') . $tname;
    }
    ...
    public function unparse_form($form, $form_data = array(), $controller = array()) {
        $tpl_data = json_decode($form->content_data,true);
